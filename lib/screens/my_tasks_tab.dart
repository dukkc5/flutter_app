// screens/my_tasks_tab.dart (Full code - Đã sửa lỗi)
...
class _MyTaskListViewState extends State<MyTaskListView> {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Lỗi tải nhiệm vụ: ${e.toString()}')),
          );
        }
      }
      if (mounted) {
  ...
  void _navigateToGroupDetail(BuildContext context, MyTaskModel myTask) {
    ...
    try {
      // 2. TÌM group object đầy đủ từ danh sách (dùng 'chatGroups' vì nó là _allGroups)
      group = groupProvider.chatGroups.firstWhere(
        (g) => g.id == myTask.groupId,
      );
    } catch (e) {
      // 3. Báo lỗi nếu không tìm thấy (ví dụ: nhóm đã bị xóa)
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Lỗi: Không tìm thấy nhóm tương ứng.')),
      );
      return;
    }

    // 4. Điều hướng bằng constructor (group: group)
    Navigator.push(
      context,
